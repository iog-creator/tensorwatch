...
class SummaryGraph(object):
    ...
    def __init__(self, model, dummy_input, apply_scope_name_workarounds=True):
        ...
        with torch.onnx.set_training(model_clone, False):
            ...
            if hasattr(jit, 'get_trace_graph'): # torch 1.1.0 or before
                ...
                raise RuntimeError(
                    f'torch version {torch.__version__} has internal changes that are not supported yet'
                )
            ...
            for node in nodes:
                ...
                if kind == 'aten::dropout':
                    ...
                    if kind == 'aten::addmm':
                        aten_addmm_nodes_scope_names.append(prev_non_dropout_op.scopeName())
            ...
            for node in graph.nodes():
                ...
                new_op['name'] = 'top_level_op' if len(module_name) == 0 else module_name
                ...
                if same_module_cnt := len(self.module_ops_map[module_name]):
                ...
                msglogger.debug(
                    f"new sgraph node - Scope name: {new_op['orig-name']} ; Type: {new_op['type']} ; Display name {new_op['name']}"
                )
    ...
    def predecessors_f(self, node_name, predecessors_types, done_list=None, logging=None, denorm_names=True):
        ...
        if node is None:
            msglogger.warning(f"predecessors_f: Could not find node {node_name}")
            return []
    ...
    def successors_f(self, node_name, successors_types, done_list=None, logging=None, denorm_names=True):
        ...
        if node is None:
            ...
            node = self.find_param(node_name)
        if node is None:
            msglogger.warning(f"successors_f: Could not find node {node_name}")
            return []
...
class _DistillerModuleList(object):
    ...
    def __repr__(self):
        ...
        for idx, module in enumerate(self._modules):
            ...
            child_lines.append(f'({str(idx)}): {mod_str}')
...
def _named_modules_with_duplicates(module, prefix=''):
    ...
    for name, submodule in module._modules.items():
        ...
        yield from _named_modules_with_duplicates(submodule, submodule_prefix)
